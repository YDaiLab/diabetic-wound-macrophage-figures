...
mat <- GetAssayData(cells, assay = "RNA", layer = "scale.data")
# ----------------------------------------------------------------------
df <- load_df()
...
p1 <- dat %>%
  ...
  theme(
    ...
    legend.title = element_text(size = fs_small),
    legend.margin = margin(),
    strip.text = element_text(size = fs_base),
...
save_figure(output_dir, "umaps_gene_features", plot = combined, width = 7.1, height = 6.5)
